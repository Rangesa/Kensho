# [ARCHIVED] Kensho MCP 実装言語比較ガイド

> **Note:** このドキュメントはアーカイブされました。検討の結果、**Rust** が採用され、Go/C実装案は破棄されました。

# Kensho MCP 実装言語比較ガイド
...
```bash
cargo build --release
# 単一バイナリ生成: target/release/kensho-mcp
```
...
# 2. テスト
./target/release/kensho-mcp analyze-binary /bin/ls

# 3. Claude Codeに統合
echo '{
  "mcpServers": {
    "ghidra": {
      "command": "'$(pwd)'/target/release/kensho-mcp"
    }
  }
